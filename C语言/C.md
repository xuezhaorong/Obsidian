...
所以 -8.25
符号位为：1
指数位为：3+127=130=10000010 使用偏移法，+127解决负指数没有符号位存储的问题
尾数位为：00001，
则内存表示为：
...
**浮点型的上溢：**
在单精度浮点数（32位）的情况下，最大可表示值为约 `3.4 × 10^38`。如果结果超出这个范围，通常会被处理为 `inf`。

**浮点型的下溢：**
除以一个非常小的数或非常大的数时，会遇到 **下溢（underflow）** 的问题。在这种情况下，数值会进入 **非正规数（subnormal）** 的范围。

指数位的范围为 `-126` 到 `127`（用偏移量127表示的范围是 `1` 到 `254`）。
...
例如：
假设最小的正常数是： 1.000000000000000000000002×2−1261.00000000000000000000000_2 \times 2^{-126}1.000000000000000000000002​×2−126

将这个数除以 2 后，我们得到： 0.100000000000000000000002×2−1260.10000000000000000000000_2 \times 2^{-126}0.100000000000000000000002​×2−126

因为尾数不再能表示 `1.0000...` 这种形式，而是 `0.1000...`，所以它变成了非正规数。
继续下溢：

如果我们继续将这个非正规数除以 2，尾数会进一步右移，最终所有的位会变成 `0`，结果是： 0.000000000000000000000002×2−126=00.00000000000000000000000_2 \times 2^{-126} = 00.000000000000000000000002​×2−126=0
...
但是，**问题在于浮点数精度的限制**。对于一个像 `2.0e20` 这么大的数，计算机只能存储前 6-7 位有效数字，后面的数字则被舍入。因此，当我们加 `1` 时，计算机没有足够的位数来存储这个变化。`1` 的影响太小，它在浮点数的表示中被舍掉了。

当较小的数加上1时
- `2.0e4` 等于 `20000`，比 `2.0e20` 小得多。
- 当我们对 `20000` 加上 `1` 时，得到的结果是 `20001`。这个时候，`1` 改变的是第5位数字。

在这种情况下，浮点数的有效位数 **足够** 表示 `20001`，因为这只是一个 5 位数，而 `float` 类型可以存储 **6到7位** 的有效数字。由于精度足够，计算结果是正确的。
...
浮点运算可能得到的结果是一个 **接近零的值**，但不是精确的零。这个小误差就是所谓的 **舍入误差**。