...
安装编译工具：
```bash
sudo apt install g++-aarch64-linux-gnu g++ cmake gperf bison flex nodejs pkg-config zlib1g zlib1g-dev python3 xz-utils perl cpanminus ninja-build libnss3-dev
``` 
...
输入配置指令：
```bash
../configure -release -opensource -confirm-license -prefix /home/xuezhaorong/Software/Qt -nomake tests -nomake examples -opengl es2 -skip qtvirtualkeyboard -skip qt3d -skip qtquick3d -skip qttools -skip qtscript -skip qtlocation -skip qtwayland -skip qtwebengine 
```
