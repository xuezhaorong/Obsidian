...
export const useSoildStore = defineStore('soild', () => {  
    ...
    // 保存SSE控制器  
    const sseControllers_humidity = ref(null);  
    ...
    async function getHumidityWithSSE(fid){  
        // 先关闭现有的连接  
        if(sseControllers_humidity.value) {  
            sseControllers_humidity.value.abort();  
        }  
  
        sseControllers_humidity.value = new AbortController();  
  
        await fetchEventSource(`/sse/soild/getHumidity?fid=${fid}`,{  
            signal: sseControllers_humidity.value.signal,  
